...
(function () {
  ...
  Object.assign(I18N.uz, {
    ...
    'wc.auth.sub': 'Ro‘yxatdan o‘tgach 1 oy davomida Standart tarifdan bepul foydalanasiz. Keyin oyiga {price} so‘m — obunani istalgan vaqtda bekor qilish mumkin.',
    'wc.auth.later': 'Hozircha o‘tkazib yuborish',
    'wc.welcome': 'Xush kelibsiz! 1 oy bepul boshlandi 🎁'
  });
  Object.assign(I18N.ru, {
    ...
    'wc.next': 'Далее',
    'wc.live': 'LIVE',
    'wc.trial': '1 месяц бесплатно',
    'wc.auth.title': 'Зарегистрируйтесь или войдите',
    'wc.auth.sub': 'После регистрации 1 месяц тарифа Стандарт бесплатно. Затем {price} сум в месяц — подписку можно отменить в любой момент.',
    'wc.auth.later': 'Пропустить пока',
    'wc.welcome': 'Добро пожаловать! Бесплатный месяц начался 🎁'
  });
  ...
  /* Kolleaj uchun tik posterlar (o'z rasmlarimiz — tez yuklanadi), reyting bo'yicha */
  const posters = MOVIES
    .filter(m => m.poster && m.poster.startsWith('images/') && !m.poster.includes('/uz/') && m.franchise !== 'konsert')
    .sort((a, b) => (b.rating || 0) - (a.rating || 0))
    .map(m => m.poster);
  const uzPoster = (MOVIES.find(m => m.franchise === 'uzbek' && m.poster) || {}).poster;
  ...
  function render() {
    ...
    root.querySelector('#wcDyn').innerHTML = auth ? `
        <section class="wc-auth">
          ...
          <button class="wc-google" id="wcGoogle" type="button">${ICON.google}<span>${t('acc.googleBtn')}</span></button>
